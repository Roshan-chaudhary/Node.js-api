...
exports.createUser = async (req, res) => {
    ...
    const { active } = req.body; // New field for active status
  
    try {
      // Set active to false if not provided in the request body
      const isActive = typeof active !== 'undefined' ? active : false;
  
      if (title && des && filename) {
        const newUser = new userModel({
          title,
          des,
          image: filename,
          active: isActive // Set to the value of isActive
        });
  ...
  exports.updateUserById = async (req, res) => {
    ...
    try {
        ...
        if (req.file) {
            // If file is uploaded, extract filename
